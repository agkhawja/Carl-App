...
class SugaryDrinks extends StatefulWidget {
  ...
  final String? initialSugaryDrinks;
  const SugaryDrinks(
      {super.key,
      this.selectedGender,
      this.selectedDate,
...
class _SugaryDrinksState extends State<SugaryDrinks> {
  ...
  final List<String> activityLevels = [
    "Never or rarely",
    "Occasionally (1-2x per week)",
    "Regularly (3-5x per week)",
    "Daily (6+ times per week)",
  ];
  @override
  void initState() {
    super.initState();
    if (widget.initialSugaryDrinks != null) {
      selectedActivity = widget.initialSugaryDrinks;
    }
  }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              "Do you regularly consume sugary drinks (sodas, commercial juices, energy drinks)?",
                ...
                onPressed: () {
                  if (selectedActivity == null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                            "Please select sugary drinks content you take."),
                      ),
                    );
                  } else {
                    print("Selected sugary drinks content: $selectedActivity");
                    // Go to next screen
                    Navigator.push(context, MaterialPageRoute(
                      builder: (context) {
                        return DietarySupplements(
